...
- **Match format** — web UI settings offer "Best of (Legs)" as [1, 3, 5], which are converted to `legs_to_win` via `ceil(N/2)` (so Best of 3 → first to 2 legs, Best of 5 → first to 3 legs). The backend `_win_leg()` checks `p.legs >= legs_to_win`; once a player crosses that threshold, their `sets` increments and all players' `legs` reset to 0 for the next set. If `p.sets >= sets_to_win`, the match ends.
...
- `audio.js` — sound effects keyed by game event; see **Speech & TTS** below
...
### Speech & TTS (`audio.js`, `commentary.js`, `scripts.js`)

MC/referee voice is synthesised via **Kokoro neural TTS** (in-browser WASM, `kokoro-js@1.2.1`, model `Kokoro-82M-v1.0-ONNX`) or falls back to browser `speechSynthesis` (system voices). Key behaviours:

- **Managed speech queue** (`_drainSpeech()`) serialises utterances one-at-a-time so they never overlap. Lines older than their TTL (3000ms default) are dropped if droppable, so stale commentary is never spoken. Priority lines (walk-ons, rules, game-shot) always play through.
- **SSML parsing** (`parseSSML()`) converts SSML tags into segments: `<break time="Xms"/>` becomes a `delayBefore` on the next segment (stacked breaks merge), `<prosody rate="fast|slow|0.9|…">` and `<emphasis level="strong|moderate|reduced">` adjust rate/pitch, tags are stripped and entities decoded. Clamp rate ∈ [0.5, 2] and pitch ∈ [0.5, 2].
- **Kokoro TTS caching** — synthesised audio is memoised in two tiers: **in-memory LRU** (300-entry limit, evicts oldest insertion) and **IndexedDB** (`darts_tts` db, `clips` store) for cross-reload persistence. Cache key is `voiceId|speed|text` (pitch and volume excluded — Kokoro doesn't use pitch, volume is applied at playback). On first synthesis, PCM is written through to both caches; on subsequent identical requests, cached audio plays instantly. IDB failures degrade silently (private mode, storage blocked) and never block speech.
- **Speech priming** — on Kokoro init, `primeSpeech(PRIME_PHRASES)` pre-synthesises common static lines (checkoutMiss, killerArm, game-on, 180, game-shot) so their first use is gap-free. Runs in the background without blocking.
- **Speech-queue ordering** — the visit-total announcement ("62") from `App.jsx`'s throw-animation effect is enqueued *after* the next-player intro from `CinematicGame.jsx`'s player-change effect in the same React commit (child effects run first). To preserve the correct playback order (score first), `CinematicGame` defers the intro via a 1100ms `turnIntroTimer`, letting the score land in the queue first and adding a deliberate beat between them.
- **SSML pacing** — high-frequency lines (those firing every visit: `turnIntro`, `visitTotal`, 180 calls) have no `<break>` tags and no `rate="slow"`, keeping them snappy. Breaks are reserved for mode-intro rules (purposefully slow/dramatic) and deflation calls (bust, checkout-miss, eliminations). The signature 180 call is a single rising shout with no internal pause.
...
## Darts Vision Debugging

When diagnosing darts misscoring, prioritise cross-camera alignment / consensus clustering as a likely root cause rather than chasing sharpness/rotation metrics or assuming duplicate detections. (Past dead ends: a rotation search when alignment was already fine, and a dedup "fix" that reverted a *real* Triple 7 — confirm the throw before assuming a phantom.)
